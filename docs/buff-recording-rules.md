...
## 7. 버프 문구(label · short) 작성 규칙

`label`(상세) / `short`(UI 압축)의 표기 관례. 기존 공명자 문구에서 도출.

### label 구조
```
[출처/스택/모드](세부·조건) : 효과 +값 [증가/부스트] (지속시간) (N돌)
```
- 출처를 먼저 적고 가운뎃점 `·` 으로 효과와 잇는다.
- 예: `공명 스킬 발동 시 파티 전체 피해 +20% 부스트 (4돌)`

### short 구조
```
[접두] 효과 +값 [부스트] (조건) (N돌)
```
- label의 압축판. 출처는 괄호로 짧게, 값·핵심 효과만.

### 증가 vs 부스트 단어 (최우선)
- **`_amplify`(부스트 버킷: 속성·전체·유형·취약) → label·short 둘 다 "부스트" 명시.** "증폭"·"↑"로 대체하지 말 것(단 공효 스케일 동적값은 `…↑ (공효 스케일)` 허용).
- **`_bonus` → "증가" 표기 또는 생략. "부스트"는 절대 쓰지 않는다.**
- `skill_motion_value_*`(배율, record_only)는 "부스트"가 아니라 **"배율 +N%"** 또는 취약이면 "받는 …피해 +N% 부스트"로 적는다.

### 약어 정책 (label은 풀네임, short는 약어)
| 스탯 | label | short |
|---|---|---|
| 크리티컬 확률 | 크리티컬 | 크리 |
| 크리티컬 피해 | 크리티컬 피해 | 크피 |
| 방어력 무시 | 방어력 무시 | 방무 |
| 속성 저항 무시 | 속성 저항 무시 | 저무 |
| 전체 속성 피해 | 전체 속성 피해 | 전속성피해 |
| 에코 어빌리티 피해 | 에코 어빌리티 피해 | 에코피해 |

이 문서 본문은 약어를 쓰지 않지만, **데이터의 `short` 필드는 UI 압축용이라 약어를 허용**한다. `label`은 반드시 풀네임.

### 타겟 접두
- `party` → **"파티 …"** 접두. (단 방어력 무시·속성 저항 무시처럼 "목표의 …"로 적히는 대상 디버프는 그대로 둔다.)
- `next_character` → label `반주: 다음 등장 캐릭터 …`, short `다음 캐릭터 …`.
- `self` → 접두 없음(강조 시 `(자력)`).

### 돌파 표기 — 표준은 `(N돌)`
- 신규는 `(N돌)`로 통일(문구 끝). ⚠️ 초기 캐릭터의 `CHAIN.N:` / `(공명 체인 N)` 은 **레거시**이며 소급 수정은 지시가 있을 때만.

### 기타
- `{v}`: 값이 공진/수치로 가변이면 `+{v}`(엔진이 치환). 주로 무기.
- 상시 패시브(포르테 등)는 `label`/`short` 없이 `note`만.
