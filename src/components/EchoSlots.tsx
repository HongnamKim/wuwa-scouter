...
import { EchoEditor, SUB_LABEL, SUB_OPTION_KEYS, mainOptionsFor } from './EchoEditor';
...
export function EchoSlots({ state, setState }: Props) {
  ...
  // 부옵 옵션 순서: 유효옵(★) → 공명 효율(유효옵 아니어도 사이클상 중요) → 나머지
  ...
  const costOptions: DropdownOption[] = [
    { value: '', label: '코스트' },
    ...availableCostsForSlot(state.costLayout, state.slots, active).map((c) => ({ value: String(c), label: `${c}코` })),
  ];
  const onCostChange = (v: string) => {
    ...
    const newMain = slot.main && valid.includes(slot.main) ? slot.main : defaultMainForCost(newCost);
      ...
      <div className="echo-tabs">
        {state.slots.map((s, i) => (
          <button key={i} className={i === active ? 'echo-tab active' : 'echo-tab'} onClick={() => setActive(i)}>
            ...
            <span className="echo-tab-cost">{s.cost != null ? `${s.cost}코` : '-'}</span>
          </button>
        ))}
      </div>
