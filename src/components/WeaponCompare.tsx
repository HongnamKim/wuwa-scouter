  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, margin: '8px 0' }}>
        <span className="muted" style={{ fontSize: '0.86rem' }}>내 무기 공진</span>
        <input type="range" min={1} max={5} step={1} style={{ width: 150 }}
          value={refinement} onChange={(e) => setRefinement(Number(e.target.value))} />
        <span style={{ fontWeight: 700, fontSize: '0.86rem' }}>{refinement}공진</span>
      </div>
      <p className="muted" style={{ margin: '0 0 12px', fontSize: '0.78rem', lineHeight: 1.5 }}>내 무기(현재)는 저장 공진({savedRef}공진) 고정(기준 100%). 슬라이더로 내 무기의 다른 공진을 행으로 추가합니다. 비교 무기는 {OTHER_REF}공진 기준.</p>
      <div style={{ border: '1px solid var(--cardBorder)', borderRadius: 14, overflow: 'hidden', background: 'var(--card)', maxWidth: 520 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 96px 108px' }}>
          <div className="wc-h">무기</div>
          <div className="wc-h" style={{ textAlign: 'right' }}>딜 상승 수치</div>
          <div className="wc-h" style={{ textAlign: 'right' }}>현재 대비</div>
          {sorted.map((r) => {
            const pct = (r.deal / baseDeal) * 100;
            const diff = pct - 100;
            const rowBg = r.current ? 'var(--bestbg)' : 'transparent';
            const color = r.current ? 'var(--muted)' : diff > 0 ? 'var(--good)' : diff < 0 ? 'var(--bad)' : 'var(--muted)';
            return (
              <Fragment key={r.key}>
                <div className="wc-cell" style={{ display: 'flex', alignItems: 'center', gap: 8, background: rowBg }}>
                  <img src={`/weapons/${r.id}.webp`} alt={r.name} style={{ width: 24, height: 24, objectFit: 'contain', flex: '0 0 auto' }} onError={onImgError} />
                  <span style={{ fontSize: '0.86rem', fontWeight: r.current ? 700 : 600 }}>{r.name} <span className="muted" style={{ fontWeight: 400, fontSize: '0.78rem' }}>{r.refinement}공진</span>{r.current ? ' (현재)' : ''}</span>
                </div>
                <div className="wc-cell" style={{ textAlign: 'right', fontFamily: 'var(--mono)', fontSize: '0.84rem', background: rowBg }}>{comma(r.deal)}</div>
                <div className="wc-cell" style={{ textAlign: 'right', fontFamily: 'var(--mono)', fontSize: '0.84rem', fontWeight: 700, color, background: rowBg }}>
                  {pct.toFixed(1)}%
                  <span style={{ display: 'block', fontSize: '0.72rem', fontWeight: 500 }}>{r.current ? '기준' : `${diff >= 0 ? '+' : ''}${diff.toFixed(1)}%p`}</span>
                </div>
              </Fragment>
            );
          })}
