...
/** 기본 슬롯(코스트 개수만큼): 코스트 미배정(null)으로 시작 — 사용자가 남은 코스트에서 직접 배정한다.
 *  (미리 배정하면 멀티셋이 전부 소진돼 코스트를 바꿀 수 없으므로 비워둔다) */
export function defaultSlots(layout: CostLayout): EchoSlot[] {
  return Array.from({ length: costsOf(layout).length }, () => ({
    cost: null,
    main: '' as const,
...
export function slotsFrom(layout: CostLayout, mainPrimary: MainPrimaryPick[], substats: SubstatLine[][]): EchoSlot[] {
  return costsOf(layout).map((cost, i) => ({
    cost,
 ...
 * 남은 에코(echoCount − maxPieces)를 2개씩 묶음: floor((echoCount − maxPieces) / 2).
 * echoCount = 코스트 개수(가변). 예) 5에코 악몽 1pc → 2슬롯(1+2+2), 왕관 3pc → 1슬롯(3+2), 일반 5pc → 0슬롯.
 */
export function freeTwoPieceSlots(echoSets: EchoSet[], echoCount = 5): number {
  return Math.max(0, Math.floor((echoCount - maxSetPieces(echoSets[0])) / 2));
}
